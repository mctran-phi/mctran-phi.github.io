...
({ scrollToAbout, scrollToProject, scrollToContact, aboutRef, projectRef, contactRef }) {
  ...
  const handleValue = (e) => {
    if (window.scrollY < aboutRef.current.offsetTop - 200) {
      ...
      setLabel(2);
    } else if (window.scrollY >= contactRef.current.offsetTop - 100 || window.scrollY + contactRef.current.clientHeight > $(document).height() - contactRef.current.clientHeight) {
      setLabel(3);
    }
  };
